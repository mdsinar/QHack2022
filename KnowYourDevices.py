...
def compare_circuits(num_wires, params):
    ...
    # QHACK #
    # define devices
    dev_p = qml.device('default.qubit', wires=num_wires, shots=None)
    dev_m = qml.device('default.mixed', wires=num_wires, shots=None)
    ...
    def pure_circuit():
        ...
        w_count = 0
        pure_params = params[0]
        while w_count < num_wires:
            qml.RY(pure_params[0+w_count], wires=w_count)
            w_count += 1
    ...
    def mixed_circuit():
        ...
        # create the circuit here
        w_count = 0
        mixed_params = params[1]
        while w_count < num_wires:
            qml.RY(mixed_params[0+w_count], wires=w_count)
            w_count += 1
